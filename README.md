...
[![Latest release](https://img.shields.io/github/v/release/YuiKira/CS2-Bot-Improver?display_name=tag&sort=semver)](https://github.com/YuiKira/CS2-Bot-Improver/releases/latest)
[![Release downloads](https://img.shields.io/github/downloads/YuiKira/CS2-Bot-Improver/total)](https://github.com/YuiKira/CS2-Bot-Improver/releases)
...
Download the Windows package from the **[latest release](https://github.com/YuiKira/CS2-Bot-Improver/releases/latest)**.
...
| `bot_randomizer_reload` | Reload the custom equipment configuration. |
...
<details>
<summary><strong>Where are custom equipment settings stored?</strong></summary>

`CS2 Bot Tools.exe` saves settings to `game/csgo/addons/counterstrikesharp/plugins/BotRandomizer/BotRandomizer.custom.json`. Each save also keeps the previous version as `BotRandomizer.custom.backup.json` in the same directory.

Release packages contain only `BotRandomizer.custom.example.json`, so extracting an update over an existing installation does not overwrite the user's weapon, knife, wear, seed, or StatTrak selections.
...
The uninstaller removes this project's files and generated equipment configuration while leaving unrelated plugin files untouched.
...
CS2 clients cache a knife's material when the entity is transmitted for the first time. Updating economy attributes after that snapshot may change the displayed skin name without changing the material. `BotRandomizer` therefore withholds newly spawned knife entities in `CheckTransmit`, applies the item definition and economy attributes, and only then releases the entity. Keep this first-transmission barrier for `subclass_create` knives and bot spawn knives; delayed attribute writes alone are insufficient.
