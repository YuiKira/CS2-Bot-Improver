...
[![最新版本](https://img.shields.io/github/v/release/YuiKira/CS2-Bot-Improver?display_name=tag&sort=semver)](https://github.com/YuiKira/CS2-Bot-Improver/releases/latest)
[![累计下载](https://img.shields.io/github/downloads/YuiKira/CS2-Bot-Improver/total)](https://github.com/YuiKira/CS2-Bot-Improver/releases)
...
Windows 用户请前往 **[最新版本](https://github.com/YuiKira/CS2-Bot-Improver/releases/latest)** 页面下载安装包。

### Windows

1. 下载并解压 **release-windows.zip**。

2. 在 Steam 库中打开 CS2 页面，点击齿轮图标，选择**管理 → 浏览本地文件**，然后依次进入 `game` 和 `csgo` 文件夹。
...
不会。从本仓库 [Releases](https://github.com/YuiKira/CS2-Bot-Improver/releases) 下载的插件用于离线 Bot 对局，不会修改真人玩家的 Steam 库存。
...
<details>
<summary><strong>装备面板的配置保存在哪里？</strong></summary>

`CS2 Bot Tools.exe` 将配置保存到 `game/csgo/addons/counterstrikesharp/plugins/BotRandomizer/BotRandomizer.custom.json`，每次保存时还会在同一目录保留上一版 `BotRandomizer.custom.backup.json`。发布包不包含运行时配置，因此覆盖更新不会清空用户选择。

</details>

<details>
<summary><strong>如何在 Windows 上完整卸载？</strong></summary>

1. 完全退出 CS2 和 `Panel v1.4.3.exe`。
2. 从 `game/csgo` 运行 `CS2 Bot Tools.exe`。
3. 点击左下角的**卸载 CS2-Bot-Improver**，连续确认两次。
4. 卸载完成后，在 Steam 的 CS2 属性中打开**已安装文件**并验证游戏文件完整性。

卸载程序会删除本项目文件和装备配置，但不会删除其他插件自行添加的文件。
